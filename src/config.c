...
bool config_save(APP_CONFIG* cfg) {
    ...
    if (cfg->disk_count > MAX_DISKS) cfg->disk_count = MAX_DISKS;
    fwprintf(f, L"  \"disks\": [\n");
...
bool config_load(APP_CONFIG* cfg) {
    ...
    while (fgetws(line, 512, f)) {
        ...
        else if (wcsstr(p, L"\"pool_mb\":")) { int v; if (swscanf(p, L" \"pool_mb\": %d,", &v) >= 1) { cfg->pool_mb = (uint32_t)v; parsed = true; } }
        ...
        else if (wcsstr(p, L"\"language\":")) { char lang[8] = {0}; if (swscanf(p, L" \"language\": \"%7[^\"]\",", lang) >= 1) { strcpy(cfg->language, lang); parsed = true; } }
