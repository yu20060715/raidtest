...
void ui_get_disk_summary(UI_DISK_SUMMARY* out) {
    ...
    if (!g_state.physical_disks) return;
    out->count = g_state.physical_count;
    uint64_t total_mb = 0;
    uint32_t sel = 0, speed_sum = 0, speed_n = 0;
    for (uint32_t i = 0; i < g_state.physical_count; i++) {
        DISK_INFO* d = g_state.physical_disks[i];
        if (d->selected) sel++;
...
void ui_get_volume_info(UI_VOLUME_INFO* out) {
    memset(out, 0, sizeof(*out));
    if (!g_state.volume_valid) return;
    STRIPE_VOLUME* vol = &g_state.volume;
    out->exists = true;
    ...
    out->mounted = g_state.mounted;
    out->cache_enabled = vol->cache_enabled;
    out->cache_mb = g_state.cache_mb;
...
uint32_t ui_get_state(void) {
    return (uint32_t)g_state.state;
}

const char* ui_get_state_str(void) {
    return raid_state_str(g_state.state);
}
