...
cd /d "%~dp0"
set "SCRIPT_DIR=%CD%"
for /f "delims=" %%i in ('C:\msys64\usr\bin\cygpath -u "%SCRIPT_DIR%"') do set "UNIX_DIR=%%i"
...
:: Step 3: link with g++
C:\msys64\usr\bin\bash.exe --login -c "export PATH='/mingw64/bin:/usr/bin:/c/Windows/System32'; cd '%UNIX_DIR%' && g++ -Wall -O2 build/*.o -o raidtest_winfsp.exe -L. -lwinfsp-x64 -lole32 -lshell32 -lshlwapi -lws2_32 -ladvapi32 -lntdll -lcomctl32 -lgdi32 -ldwmapi -ld3d11 -ldxgi -ld3dcompiler -static-libgcc -static-libstdc++ -static 2>&1"
if errorlevel 1 (
  echo Link FAILED
  pause
  exit /b 1
...
C:\msys64\usr\bin\bash.exe --login -c "export PATH='/mingw64/bin:/usr/bin:/c/Windows/System32'; cd '%UNIX_DIR%' && gcc -Wall -O2 -Isrc src/test_runner.c src/test_common.c src/test_stripe.c src/test_mirror.c src/test_cache.c src/test_journal.c src/test_superblock.c src/stripe_engine.c src/mirror_engine.c src/storage_common.c src/ram_cache.c src/logger.c src/journal.c src/pool_io.c src/superblock.c src/crc32.c src/uuid.c src/profiler.c -o raidtest_tests.exe -static-libgcc 2>&1"
if errorlevel 1 (
